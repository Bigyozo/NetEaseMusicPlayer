...
- 技术栈 Angular 21、ng-zorro-antd 21。
...
1. 网易云音乐 API 部署完成后，根据网易云音乐 API 的 IP 与端口设置 `proxyconfig.js` 文件中的 target 属性。
2. npm install 后启动项目。
...
1. 将编译生成的 `www/browser` 目录、`nginx.conf.template` 与 `Dockerfile` 文件放入 Linux 服务器。
2. 生成 docker 镜像。
